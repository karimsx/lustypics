...
import React, {
    useEffect, useState, useContext, ReactNode, useRef
} from 'react'
...
export const ClientContext = React.createContext<GraphQLClient | null>(null)
...
export function useQuery<T>(query: any, variables?: object): IQueryState<T> {
    ...
    useEffect(() => {
        ...
        const refetch = () => {
            ...
            return graphQLClient.request<T>(queryAsString, variables).then((data: T) => {
        ...
        graphQLClient.request<T>(queryAsString, variables).then((data: T) => {
            if (isMounted.current) {
                setState({data, loading: false, refetch})
            }
        }, (res) => {
